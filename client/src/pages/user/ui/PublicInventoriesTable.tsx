...
export default function PublicInventoriesTable() {
  const { t } = useTranslation('common')
  const navigate = useNavigate()
  const [selection, setSelection] = useState<DataTableSelection>([])

  const { data, isLoading, isError } = useQuery({
    queryKey: ['inventories', 'public'],
      ...
      >
        <Stack spacing={0.2} sx={{ minWidth: 0 }}>
          ...
          <Typography variant="body2" color="text.secondary">
            {t('workspace.publicInventoriesSubtitle')}
          </Typography>
        </Stack>
  ...
  return (
    <Panel>
      <DataTable<Row>
        ...
        toolbar={toolbar}
        emptyTitle={t('workspace.noPublicInventories')}
      />
    </Panel>
  )
}